...
const validatePin = (pin) => {
  // console.log(pin, 'type is ', typeof pin);
  let Regex = /[a-z]|[A-Z]/;
  ...
  // (typeof pin === 'number')&&
  if((pin.toString().length === 4)&&(test === false) ){
    return true;
...
const validatePhoneNumber = (phoneNumber) => {
  ...
  let test = Regex.test('222 222-2222');
  let letterTest = letter.test(phoneNumber);
  console.log('Regex test ', test);
  if((test === true)&&(letterTest===false)){
    return true
  }
  else{
    return false;
