...
const hasChildrenValues = (arr, character) => {
  ...
  let isTrue = false;
  arr.forEach(index =>{
    if((index.children.length > 0) && (index.name === character) ){
      isTrue = true;
    }
  })
  return isTrue;
...
const houseSize = (arr) => {
  const sizes = [];
arr.forEach( index => {
if (index.spouse === null){
  sizes.push({  house:  index.house, members: (1+index.children.length)})
}
else{
  sizes.push({  house:  index.house, members: (2+index.children.length)})
