...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach( (index)=> {
    ...
    result.push(obj[2]);
      ...
      result.push(obj[2]+' '+obj[3]+' '+obj[4])
    }
    else{
      result.push(obj[2]+' '+obj[3]);
    }
  })
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  else if(numberOfCharacters > len){
    return str='';
  }
...
const removeVowels = (str) => {
  let result ='';
  let regex =/a|A|e|E|i|I|o|U/g
for (var i in str){
  if(regex.test(str[i])===false){
    var holder = str[i].slice('0,1');
...
const extractVowels = (str) => {
  ...
  for (var i in str){
    if((regex.test(str[i])===false) && (str[i] !== 'i') ){
      // console.log('false    '+ str[i])
      var holder1 = str[i].split("");
      result1 = result1+holder1;
      
    }
  else {
    var holder = str[i].split("");
    result2.push(holder);
    
  }
}
result2 =result2.sort();
result2.forEach((index)=>{
  vowels = vowels + index;
})
arr[0]=result1;
arr[1]=vowels;
