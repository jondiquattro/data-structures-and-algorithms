...
const firstLetters = (arr) => {
  const result =[];
  arr.forEach(idx =>{result.push(idx[0])})
  return result;
};
...
Write a function named standardizePhoneNumbers that takes in an array of phone number strings in (XXX) XXX-XXXX format and returns an array with 
...
const allHappy = (arr) => {
    ...
    let regex = /[:)]/;
    let count =0;
   arr.forEach(idx =>{if(regex.test(idx)){
    count ++
    }
    if (count ==arr.length){
      result = true
