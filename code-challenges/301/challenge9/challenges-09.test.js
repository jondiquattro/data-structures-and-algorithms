...
const countNumberOfElements = (arr) => {
  ...
  // },0)
  let x =arr.reduce((acc,val,idx)=>{return idx+1},0);
...
const returnNames = (arr) => {
  const arrx=[];
  let x =starWarsData.reduce((acc,val,idx)=>{
    arrx.push(val.name)},0);
    return arrx;
};
...
const reversedString = (arr) => {
  ...
  strArr.reduce((acc,val,idx)=>{
    arrx.unshift(val)},0)
   let str = arrx.toString().replace(/,/g , "");
   return str;}
...
const countNumberOfChildren = (arr) => {
  const arrx=[];
  let x =characters.reduce((acc,val,idx)=>{
    if(val.children){
    arrx.push(val.children.length)
    }
    },0);
    // const reducer = (accumulator, currentValue) => accumulator + currentValue;
    return arrx.reduce( (acc, val,)=>{return acc + val},0)

// return arrx.reduce(reducer);
};
...
const calculateAverage = (arr) => {
  ...
  return arr.reduce( (acc, val,)=>{return acc + val},0)/arr.length
...
const countPrimeNumbers = (arr) => {
  const prime =[];
  arr.reduce( (acc, val)=>{if(isPrime(val)){prime.push(val)}},0)
  return prime.length;
}
