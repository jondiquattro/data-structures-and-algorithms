...
const isNum = (input) => {
  ...
  let testNum =numRegex.test(input);
  let testStr =strRegex.test(input);
  let flag = false;
...
const isCapitalized = (str) => {
  let numRegex = /\d/g;
  let strRegex = /\w/g;
  let capRegex =/[A-Z]\w+/g

  let testNum =numRegex.test(str);
  let testStr =strRegex.test(str);
