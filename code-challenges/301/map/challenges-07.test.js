...
const forLoopTwoToThe = (arr) => {
  const result =[];
  arr.forEach(element => {//outer loop at each element in array
    if (element > (-1)){
    let power = 1;
    for(let i = 0; i< element; i++) //if element = 2 loop runs twice
    {
      power = 2*power;
    }
    result.push(power);
      ...
      let dec = 0;
      let power = 1;
          for(let i = 0; i< element; i++) //if element = 2 
    {
      power = 2*power;
    }
    dec = 1/power;
...
//I was having a lot of trouble figuring out how mapping works so Charles helped me figure out how to do this part of the challenge
