...
const count = (target, input) => {
  let count=[];
  input.reduce( (acc, val)=>{count.push(val.filter( test => test ===target).length) 
  return count},0)
  return count.reduce((acc, val) => {return acc + val},0)}
...
const totalSum = (input) => {
  const result =[];
  return input.reduce( (acc, val)=>{result.push(val.reduce( (acc, val)=>{return acc+val},0));
  return result.reduce( (acc, val)=>{return acc+val},0)},0)
   };
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.reduce( (acc, val)=>{
    let y =val.filter(test => !test.length)
    isANum.push(y);
    },0)
  isANum.reduce( (acc, val)=>{
    let y =val.filter(test => !(test%5))
    result.push(y);
    },0)

result.forEach( val=>{let y =val.map(idx =>Math.pow(2, idx));
    result2.push(y);
...
let findMaleAndFemale = (data) => {
  let str =''
  let y = data.filter( test => (test.gender ==='male')||(test.gender ==='female')).map(idx => idx.name).forEach(idx => {
    str += idx+' and ';
  })
